...
export const ContributionGraph = ({
  ...
  return (
    <ContributionGraphContext.Provider
      value={{ data, weeks, blockMargin, blockRadius, blockSize, fontSize, labels, labelHeight, maxLevel, totalCount, weekStart, year, width, height }}
    >
...
export const ContributionGraphBlock = ({
  ...
  if (activity.level < 0 || activity.level > maxLevel) {
    throw new RangeError(`Activity level ${activity.level} for ${activity.date} is out of range [0, ${maxLevel}].`);
  }
...
export const ContributionGraphCalendar = React.forwardRef<HTMLDivElement, ContributionGraphCalendarProps>(({
  ...
  return (
    <div ref={ref} className={cn("max-w-full overflow-x-auto", className)} {...props}>
      ...
      {new Array(maxLevel + 1).fill(undefined).map((_, level) =>
        children ? (
          ...
          <svg height={blockSize} key={level} width={blockSize}>
            ...
            <rect className={cn(THEME)} data-level={level} height={blockSize} rx={blockRadius} ry={blockRadius} width={blockSize} />
          </svg>
        )
      )}
